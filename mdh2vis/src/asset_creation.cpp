...
{
    return create_texture_asset(asset_name, "assets/textures/cube.png",
...
{
    return create_texture_asset(asset_name, "assets/textures/output_cube.png",
        { Visconfig::Assets::TextureAttributes::MagnificationLinear,
            Visconfig::Assets::TextureAttributes::MinificationLinear,
            Visconfig::Assets::TextureAttributes::GenerateMipMaps });
}
...
Visconfig::Asset create_simple_cube_mesh_asset(const std::string& asset_name)
{
    ...
    constexpr std::array<std::array<float, 4>, 8> tex_coords{
    ...
    mesh_data->texture_coords0.resize(tex_coords.size());

    std::copy(indices.begin(), indices.end(), mesh_data->indices.begin());
    std::copy(vertices.begin(), vertices.end(), mesh_data->vertices.begin());
    std::copy(tex_coords.begin(), tex_coords.end(), mesh_data->texture_coords0.begin());

    return asset;
}

Visconfig::Asset create_shader_asset(
    const std::filesystem::path& vertex_path, const std::filesystem::path& fragment_path, const std::string& asset_name)
{
    ...
    shader_data->vertex = vertex_path;
    shader_data->fragment = fragment_path;
    ...
    const std::string& asset_name, std::size_t width, std::size_t height, Visconfig::Assets::TextureFormat format)
{
    ...
    Visconfig::Asset asset{ asset_name, Visconfig::Assets::AssetType::TextureRaw, texture_data };
    ...
    std::size_t samples, Visconfig::Assets::RenderbufferFormat format)
{
    ...
    Visconfig::Asset asset{ asset_name, Visconfig::Assets::AssetType::Renderbuffer, renderbuffer_asset };
        ...
        std::string>>& attachments)
{
    ...
    Visconfig::Asset asset{ asset_name, Visconfig::Assets::AssetType::Framebuffer, buffer_data };
    ...
    std::size_t height, std::size_t samples, Visconfig::Assets::TextureFormat format)
{
    ...
    Visconfig::Asset asset{ asset_name, Visconfig::Assets::AssetType::TextureMultisampleRaw, texture_data };
    texture_data->width = width;
    texture_data->height = height;
    ...
    const std::vector<Visconfig::Assets::TextureAttributes>& attributes)
{
    ...
    Visconfig::Asset asset{ asset_name, Visconfig::Assets::AssetType::TextureFile, texture_data };
    ...
    std::size_t subdivisions_y, std::size_t line_width)
{
    ...
    auto texture_quality_multiplier{ static_cast<std::size_t>(
        interpolate_linear(min_texture_quality_multiplier, max_texture_quality_multiplier, 0.0f, 1.0f, line_ratio)) };
    ...
    stbi_write_png(name.c_str(), texture_width, texture_height, 3, texture_data.data(), texture_row_stride);
}
