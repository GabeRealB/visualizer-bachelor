...
MeshDrawingSystem::MeshDrawingSystem()
    : m_mesh_query{ EntityQuery{}.with<std::shared_ptr<Mesh>, Material, Transform, RenderLayer>() }
    , m_camera_query{ EntityQuery{}.with<Camera, Transform>() }
    , m_entity_database{}
{
}
...
void MeshDrawingSystem::run(void*)
{
    ...
    m_entity_database->enter_secure_context([&](EntityDatabaseContext& database_context) {
        auto drawable_meshes{ m_mesh_query.query(database_context) };

        m_camera_query.query(database_context).forEach<Camera, Transform>([&](Camera* camera, Transform* transform) {
            ...
            view_matrix
                = glm::toMat4(glm::inverse(transform->rotation)) * glm::translate(view_matrix, -transform->position);
